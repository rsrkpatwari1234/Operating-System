...
{
  /* checking if new_nice value lies between -20 and 20 */
  ASSERT (new_nice >= NICE_MIN && new_nice <= NICE_MAX);
  
  /* NOT SURE */
  enum intr_level old_level = intr_disable ();

  /* Setting nice value of current thread */
  struct thread *curr;
  curr = thread_current ();
  curr->nice = new_nice;

  /* recalculating current thread priority */
  thread_mlfqs_update_priority (curr);
  
  /* yielding the current thread if it is not of highest priority */
  thread_test_preemption ();
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  /* Assignment 6 : Part 1 : Part 2 started */
